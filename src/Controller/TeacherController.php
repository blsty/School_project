...
####################################################################################################
    ...
    public function formquestion()
    {
        ...
        $questionn->setTeacher($this->getUser());
...
####################################################################################################
    ...
    public function form_fill($id)
    {
        ...
        $numberOfQuestions = $request->get('numquestions');

        $user = $this->getDoctrine()->getRepository(User::class)->findAllStudents();
        $exam = $this->getDoctrine()->getRepository(Exam::class)->find($id);
        $questions = $this->getDoctrine()->getRepository(Question::class)
            ->findBy(["subject" => $exam->getSubject()]);


        if ($allStudents == 'on') {
            $exam->SetForAll($allStudents);
            $entityManager->flush();
        } else {
            $exam->SetForAll(0);
            foreach ($studentId[0] as $student) {
                $usr = $this->getDoctrine()->getRepository(User::class)->find($student);
                $exam->addUser($usr);
            }
            $entityManager->flush();
        ...
        if ($randomQuestions == 'on') {
            $questions = $this->getDoctrine()->getRepository(Question::class)
                ->findBy(["teacher" => $this->getUser(), "subject" => $exam->getSubject()]);
            shuffle($questions);
        ...
        return $this->redirectToRoute('exam_show',["id"=> $id]);
    ...
    public  function exam_show($id){
        ...
        return $this->render('teacher/exam_show.html.twig',["id"=> $id,
            "exam"=>$exam,"user"=>$user, "questions"=>$questions]);
    }
